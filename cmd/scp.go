...
import (
	...
	"github.com/fromanirh/pack8s/cmd/utils"
	"github.com/fromanirh/pack8s/docker"
...
func scp(cmd *cobra.Command, args []string) error {
	...
	cli, err := cntwrapper.NewEnvClient()
	if err != nil {
		return err
	}

	containers, err := cli.GetPrefixedContainers(prefix + "-" + containerName)
	if err != nil {
		return err
	}
	...
	sshPort, err := containers[0].GetPublicPort(utils.PortSSH)
	if err != nil {
		return err
	}
